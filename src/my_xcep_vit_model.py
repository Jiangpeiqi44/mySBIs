...
class Block_v2(nn.Module):
    def __init__(self,
        ...
        self.norm1 = norm_layer(dim)
        self.attn = Attention_v2(dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale,
                                 attn_drop_ratio=attn_drop_ratio, proj_drop_ratio=drop_ratio)
        # NOTE: drop path for stochastic depth, we shall see if this is better than dropout here
        self.drop_path = DropPath(
            drop_path_ratio) if drop_path_ratio > 0. else nn.Identity()
...
class VisionTransformer_w_consis_v2(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_c=3, num_classes=1000,
                 embed_dim=768, depth=12, num_heads=12, mlp_ratio=4.0, qkv_bias=True,
                 qk_scale=None, representation_size=None, distilled=False, drop_ratio=0.,
                 attn_drop_ratio=0., drop_path_ratio=0., embed_layer=PatchEmbed, norm_layer=None,
        ...
        super(VisionTransformer_w_consis_v2, self).__init__()
        self.num_classes = num_classes
        # num_features for consistency with other models
        self.num_features = self.embed_dim = embed_dim
        self.num_tokens = 2 if distilled else 1
        ...
        dpr = [x.item() for x in torch.linspace(0, drop_path_ratio, depth)]
        self.blocks = nn.Sequential(*[
            Block_v2(dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                     drop_ratio=drop_ratio, attn_drop_ratio=attn_drop_ratio, drop_path_ratio=dpr[
                         i],
                     norm_layer=norm_layer, act_layer=act_layer)
            for i in range(depth)
...
def vit_base_patch16_224_in21k_w_consis(num_classes: int = 21843, has_logits: bool = True, isEmbed: bool = True, drop_ratio: float = 0.,
...
def vit_base_patch16_224_in21k_w_consis_v2(num_classes: int = 21843, has_logits: bool = True, isEmbed: bool = True, drop_ratio: float = 0.,
                                           attn_drop_ratio: float = 0., drop_path_ratio: float = 0., keepEmbedWeight: bool = True):
    model = VisionTransformer_w_consis_v2(img_size=224,
                                          patch_size=16,
                                          embed_dim=768,
                                          depth=12,
                                          num_heads=12,
...
class XVit_Net(nn.Module):
    def __init__(self):
        super().__init__()
        self.vit_model = vit_base_patch16_224_in21k_wo_embed(
            num_classes=2, has_logits=False)
        ...
        self.dual_cma0 = DualCrossModalAttention(
            in_dim=728, size=14, ret_att=False)
...
class XVit_Net_RGB_branch(nn.Module):
    def __init__(self):
        ...
        self.embed_conv = nn.Conv2d(
            in_channels=728, out_channels=768, kernel_size=1)
...
class XVit_Net_RGB_consis(nn.Module):
    ...
    def forward(self, x):
        ...
        consis_map = 0.5 + 0.5*((self.K(patch_token) @ self.Q(patch_token).transpose(-2, -1)
                                 ) / (consis_map_norm@consis_map_norm.transpose(-2, -1)))
...
# 在CNN Embedding后添加PCL监督
class XVit_Net_RGB_consis_v2(nn.Module):
    def __init__(self):
        super().__init__()
        self.vit_model = vit_base_patch16_224_in21k_wo_embed(
            num_classes=2, has_logits=False)
    ...
    def forward(self, x):
        ...
        fea = fea.flatten(start_dim=2, end_dim=-1)
        # Out as [B, 768, 196]
        patch_token = fea.permute(0, 2, 1)
        # Out as [B, 196, 768]
        cls_token = self.vit_model(patch_token)
        # # consis-1
...
class Vit_Net_Two_Stream(nn.Module):
    def __init__(self):
        super().__init__()
        self.vit_model = vit_base_patch16_224_in21k_wo_embed(
            num_classes=2, has_logits=False, keep_embed_weights=True)
...
class Vit_consis_hMLP(nn.Module):
    def __init__(self):
        super().__init__()
        self.vit_model = vit_base_patch16_224_in21k_w_consis_v2(
            num_classes=2, has_logits=False, isEmbed=True)
        # consis-1
        self.K = nn.Linear(768, 768)
        self.Q = nn.Linear(768, 768)
        self.scale = 768 ** -0.5
...
class Vit_consis_v2(nn.Module):
    def __init__(self):
        ...
        self.embed_conv = nn.Conv2d(
            in_channels=728, out_channels=768, kernel_size=1)
        self.fc_embed = nn.Linear(768*2, 768)
        # consis-1
        self.K = nn.Linear(768, 768)
        self.Q = nn.Linear(768, 768)
        self.scale = 768 ** -0.5
    ...
    def forward(self, x):
        LinPrj_embed = self.vit_model.patch_embed(x)
        x = self.xception_rgb.model.fea_part1(x)
        x = self.xception_rgb.model.fea_part2(x)
        x = self.embed_conv(x)
        x = x.flatten(start_dim=2, end_dim=-1)
        # Out as [B, 768, 196]
        Conv_embed = x.permute(0, 2, 1)
        # Out as [B, 196, 768]
        embed = self.fc_embed(torch.cat((LinPrj_embed, Conv_embed), dim=2))
        cls_token, patch_token = self.vit_model(embed)
        # # consis-1
        consis_map = (self.K(patch_token) @
                      self.Q(patch_token).transpose(-2, -1)) * self.scale
        # # consis-2 add norm
    ...
    def test_time(self, x):
        ...
        # Out as [B, 196, 768]
        embed = self.fc_embed(torch.cat((LinPrj_embed, Conv_embed), dim=2))
        cls_token, _ = self.vit_model(embed)
        return cls_token

class Vit_consis_EarlyConv(nn.Module):
    def __init__(self):
        super().__init__()
        self.vit_model = vit_base_patch16_224_in21k_w_consis_v2(
...
class Vit_consis_RegionConv(nn.Module):
    def __init__(self):
        super().__init__()
        self.vit_model = vit_base_patch16_224_in21k_w_consis_v2(
            num_classes=2, has_logits=False, isEmbed=True, keepEmbedWeight=True)
        # RGB embed 1
        self.region_conv = RegionLayer(in_channels=3, grid=(8, 8))
        # consis-1
        self.K = nn.Linear(768, 768)
        self.Q = nn.Linear(768, 768)
        self.scale = 768 ** -0.5
    ...
    def forward(self, x):
        ...
        x = self.region_conv1(x)
        x = self.region_conv2(x)
        # # normal LinPrj
        cls_token, patch_token = self.vit_model(x)
        # # consis-1
        consis_map = (self.K(patch_token) @
...
class Vit_RegionConv(nn.Module):
    def __init__(self):
        super().__init__()
        self.vit_model = vit_base_patch16_224_in21k_w_consis_v2(
            num_classes=2, has_logits=False, isEmbed=True, keepEmbedWeight=True)
        # RGB embed 1
        self.pre_conv =  nn.Conv2d(in_channels=3, out_channels=32, kernel_size=9, stride=1),  # k=11
        self.region_conv = RegionLayer(in_channels=32, grid=(8, 8))
        # consis-1
        # self.K = nn.Linear(768, 768)
        # self.Q = nn.Linear(768, 768)
        # self.scale = 768 ** -0.5
...
class RegionLayer(nn.Module):
    def __init__(self, in_channels, grid=(16, 16)):

...
def main(args):
    ...
    train_dataset = SBI_Dataset(
        phase='train', image_size=image_size, n_frames=16)
    val_dataset = SBI_Dataset(phase='val', image_size=image_size, n_frames=16)
    ...
    #     pg, lr=5e-5, betas=(0.9, 0.999), weight_decay=0.1)  # 6e-5  3e-5  2e-5 wd默认1e-2
    optimizer = SAM(pg, torch.optim.AdamW, lr=5e-5, betas=(0.9, 0.999), weight_decay=0.1)
    ## add 载入已训练

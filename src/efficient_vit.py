...
class EfficientViT(nn.Module): 
    ...
    def __init__(self, config, channels=512, selected_efficient_net = 0):
        ...
        patch_dim = channels * patch_size ** 2  # 通道数 * 特征图大小

        self.patch_size = patch_size

        self.pos_embedding = nn.Parameter(torch.randn(emb_dim, 1, dim)) # 32,1,1024
        self.patch_to_embedding = nn.Linear(patch_dim, dim)
        self.cls_token = nn.Parameter(torch.randn(1, 1, dim))   #(1,1,dim=1024)
    ...
    def forward(self, img, mask=None):
        ...
        y = rearrange(x, 'b c (h p1) (w p2) -> b (h w) (p1 p2 c)', p1 = p, p2 = p)  # B,1280,7,7  ->  # B,1,7*7*1280
        #y2 = rearrange(x2, 'b c (h p1) (w p2) -> b (h w) (p1 p2 c)', p1 = p, p2 = p)
        y = self.patch_to_embedding(y)  # Linear(7*7*1280,dim=1024) -> B,1,1024
        cls_tokens = self.cls_token.expand(x.shape[0], -1, -1)  # B,1,1024
        x = torch.cat((cls_tokens, y), 1)   # B,2,1024
        shape=x.shape[0]
        x += self.pos_embedding[0:shape]    # pos_embedding: # 32,1,1024
        x = self.dropout(x)

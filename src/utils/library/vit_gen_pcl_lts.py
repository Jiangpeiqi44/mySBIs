...
def random_get_hull_debug(landmark, img1):
        mask = dfl_full(landmarks=landmark.astype(
            'int32'), face=img1, channels=3).mask
        Image.fromarray(np.uint8(mask)).convert('RGB').save("imgs/mask_1.png")
        mask = extended(landmarks=landmark.astype(
            'int32'), face=img1, channels=3).mask
        Image.fromarray(np.uint8(mask)).convert('RGB').save("imgs/mask_2.png")
        mask = components(landmarks=landmark.astype(
            'int32'), face=img1, channels=3).mask
        Image.fromarray(np.uint8(mask)).convert('RGB').save("imgs/mask_3.png")
        mask = facehull(landmarks=landmark.astype(
            'int32'), face=img1, channels=3).mask
        Image.fromarray(np.uint8(mask)).convert('RGB').save("imgs/mask_4.png")
...
class BIOnlineGeneration():
    def __init__(self,phase):
        ...
        with open('src/utils/library/ff_lm_{}_clean.json'.format(phase), 'r') as f:

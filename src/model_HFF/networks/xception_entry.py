...
PRETAINED_WEIGHT_PATH = 'src/model_HFF/networks/xception-b5690688.pth'
...
class TransferModel(nn.Module):
    ...
    def __init__(self, modelchoice, num_out_classes=2, dropout=0.0,
                 weight_norm=False, return_fea=False, inc=3, type='Full'):
        ...
        if modelchoice == 'xception':
            ...
            def return_pytorch04_xception_entry(pretrained=True):
                # Raises warning "src not broadcastable to dst" but thats fine
                model = xception(pretrained=False)
                if pretrained:
                    # Load model in torch 0.4+
                    model.fc = model.last_linear
                    del model.last_linear, model.fc, model.block4, model.block5, model.block6, model.block7, model.block8, model.block9, model.block10, model.block11, model.block12, model.conv3, model.conv4, model.bn3, model.bn4
                    state_dict = torch.load(
                        PRETAINED_WEIGHT_PATH)
                    for name, weights in state_dict.items():
                        if 'pointwise' in name:
            ...
            if type=='Full':
                ...
                self.model = return_pytorch04_xception_entry()
            else:
...
if __name__ == '__main__':
    ...
    model_entry = TransferModel('xception',type='Entry')
    # print(model)
    # image_size = 224
    # batch_size = 1
    ...
    # out = model(dummy)
    # print(out.size())
    x = model_entry.model.fea_part1(dummy)
    print(x.shape)
    x = model_entry.model.fea_part2(x)
    print(x.shape)

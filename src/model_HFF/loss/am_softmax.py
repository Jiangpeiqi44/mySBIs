...
class AngleSimpleLinear(nn.Module):
    ...
    def forward(self, x):
        cos_theta = F.normalize(x, dim=1).mm(F.normalize(self.weight.to(device), dim=0))
...
if __name__ == '__main__':
    a = torch.rand(16, 2048)
    angel = AngleSimpleLinear(2048, 2)
    b = angel(a)
    loss = AMSoftmaxLoss()
    label = torch.zeros(16, dtype=torch.int64)
    # print(label.shape)
    out = loss(b, label)
    print(out)
